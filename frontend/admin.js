...
function showSection(id) {
    sections.forEach((s) => {
        if (s.id === id) {
            ...
            if (id === "contact-customers") {
                loadUsers(); // Refresh inbox when opening
            }
        } else {
...
function renderOrders() {
    ...
    if (ordersBody) {
        ...
        ${["PENDING", "IN_PROGRESS", "READY", "CANCELLED"].map((status) =>
...
async function loadServices() {
    ...
    try {
        const names = await api.get("/api/catalog/services");
        const units = await api.get("/api/catalog/units");
        servicesBody.innerHTML = names.map((name, idx) => `
            <tr>
                <td>#${idx + 1}</td>
                <td><strong>${name}</strong></td>
                <td>LKR 1,500 <span class="muted small-text">(Est.)</span></td>
                <td>${units[idx] || units[0] || "Per Kg"}</td>
                <td>
                    <button class="btn btn-pill" style="background:var(--muted); box-shadow:none;">Edit</button>
                </td>
            </tr>
        `).join("");
    } catch (error) {
...
function renderMessageUsers() {
    ...
    <button class="btn ${selectedMessageUser === user.id ? 'active' : ''}" data-message-user="${user.id}" style="width:100%; justify-content:flex-start;">
      ${user.name}
    </button>
  </li>`).join("");
}
...
messageUsers?.addEventListener("click", (event) => {
    const target = event.target.closest("[data-message-user]");
    if (!target) return;
...
async function loadMessages() {
    const threadHeader = document.querySelector(".thread-meta h3");
    if (!selectedMessageUser) {
        if (threadHeader) threadHeader.textContent = "Conversation";
        messageList.innerHTML = `<p style="color:var(--muted);">Select a customer to view messages.</p>`;
        return;
    }
    const customer = users.find(u => u.id === selectedMessageUser);
    if (threadHeader && customer) threadHeader.textContent = `Conversation with ${customer.name}`;
...
async function initialiseFilters() {
    ...
    filterStatus.innerHTML = `<option value="">All statuses</option>` +
        ["PENDING", "IN_PROGRESS", "READY", "CANCELLED"].map((status) => `<option value="${status}">${status}</option>`).join("");
}
